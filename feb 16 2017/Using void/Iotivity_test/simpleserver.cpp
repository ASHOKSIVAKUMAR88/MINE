...
std::string  manufacturerName = "ramya_vt";
...
std::string  deviceName = "IoTivity Simple Server ashok p";
...
public:
    ...
    OCResourceHandle m_resourceHandle;///////Handle to an OCResource object owned by the OCStack
    OCRepresentation m_lightRep;  ////works like a dictionary
    ObservationIds m_interestedObservers;////list of observation IDs  Unique identifier for each observation request

public:
    /// Constructor
    LightResource()
        :m_name("ASHOK SIVA KUMAR PUJALA"), m_state(false), m_power(0), m_lightUri("/a/ashok"),
	...
	cout<<boolalpha;
		...
		if(strcmp(s.c_str(),"true")==0)
 		...
 		cout << "\t\t\t\t" << "state: " << m_state << endl;
            ...
            {
		rep.getValue("power",s);
		
		m_power = atoi(s.c_str());
		cout << "\t\t\t\t" << "power: " << m_power << endl;
            }
