...
void found_resource(std::shared_ptr<OCResource> resource)
{
	...
	{ 		
		...
		//local_info->handler = (Resource_list *)malloc(sizeof(Resource_list)); 
...
c_json DM_Get(Resource_info* resource1, onGet_ptr1 call_back)
{
	...
	cout<<resource1->name<<endl;
    	((Resource_list*)resource1->handler)->DM_Resource->get(test, &on_Get);
...

{
	...
	int i,j;
			for(i=0;i<=in_data.NumOfElements;i++)
			{
				...
				((Resource_list*)resource1->handler)->DM_Resource->put(rep, QueryParamsMap(), &onPut);
				DM_GET.wait(DM_WAIT);
				call_back(in_data);
