...
final_t final_table[MAX_SIZE]={0};
...
key_value_pair_t *form_key_value_pair (info * tbl)
{
  int index = 0;
  int i;
  int primary_index;
  ...
  key_value_pair_t *pKeyValue_LocalNode;
  key_value_pair_t *pKeyValue_Temp;
  unsigned char *pcKey;
...
void print_char_table (info * table)
{
  int i, j;
  for (i = 0; i < table_index; i++)
    {
      printf ("table [%d] Handle:%.2x Type:", i, table[i].handle);
      for (j = 15; j >= 0; j--)
        printf ("%.2x ", table[i].type[j]);

      for (j = 0; j < 16; j++)
        printf ("%.2x.", table[i].value[j]);
...
final_t final_data(key_value_pair_t *key_val_pairs)
{
    int m=0;
    for(m=0;m<final_index;m++)
    {
        strcpy(final_table[m].key, key_val_pairs ->key);
        memcpy(final_table[m].value, key_val_pairs ->value);
    }
}
int main (int argc, char *argv[])
{
  ...
  //print_char_table (characteristic_table);
  pairs = form_key_value_pair(characteristic_table);
  while (pairs != NULL)
    {
      printf("final_handle: %.4X\n", pairs->handle_res);
      printf ("final_key %s \n", pairs->key);
      printf("final_value:");
      for (i=0; i<20;i++) printf("%.2X ",pairs->value[i]);
      printf("\n");
      pairs = pairs->pNext;
    }
   final_data(pairs);
