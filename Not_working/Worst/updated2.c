...
key_value_pair_t *pKeyValue_Global_new = NULL;
key_value_pair_t *pKeyValue_Global_new1 = NULL;
  ...
  while ((primary_index = get_table_index_by_uuid (0x2800, index)) != 0xffff)
    {
      ...
      memset(wpdu, '\0', 512);
      memset(rpdu, '\0', 512);
      wpdu[0] = BT_ATT_OP_READ_REQ;//0x0A
      ...
      {
//      while(rpdu[0] != ATT_OP_READ_RESP)//0x0B
//      {
//          memset(rpdu, '\0', 512);
//          memset(wpdu, '\0', 512);
//          size = write(fd, wpdu, 3);
...
//      }
      ...
      printf("len in form_key_value_pair %d rpdu %.2x\n",len,rpdu[0]);
      for(int g=0; g<len; g++)
      {
          printf("rpdu[%d]= %.2x \n",g,rpdu[g]);
      }
...
key_value_pair_t * Write_data(int fd,info * tbl)
{
    unsigned char wpdu[512];
    unsigned char rpdu[512];
    unsigned int len = 0;
    ...
    for(i=0;i<table_index;i++)
    {
		...
		//printf("============================\n");
                ...
                memset(wpdu, '\0', 512);
                memset(rpdu, '\0', 512);
                wpdu[0] = BT_ATT_OP_WRITE_REQ; //BT_ATT_OP_WRITE_REQ 0x12
                ...
                len = read (fd, rpdu, 512);
                {
//                    while(rpdu[0] != BT_ATT_OP_WRITE_RSP)//BT_ATT_OP_WRITE_RSP 0x13
//                {
//                    memset(rpdu, '\0', 512);
//                    len = read (fd, rpdu, 512);
//                    sleep(1);
//                }
                ...
                printf("len in write_data %d rpdu %.2x\n",len,rpdu[0]);
                memset (tbl[key_value_pair_index].value, 0, sizeof (tbl[key_value_pair_index].value));
                memcpy(tbl[key_value_pair_index].value,rpdu,len);
...
//                }
                ...
                printf("len in write_to_pair %d rpdu %.2x\n",len,rpdu[0]);
                memset (tbl[key_value_pair_index].value, 0, sizeof (tbl[key_value_pair_index].value));
                memcpy(tbl[key_value_pair_index].value,rpdu,len);
...
int main (int argc, char *argv[])
{
  ...
  key_value_pair_t *pairs, *pairs1;
  key_value_pair_t *pairs_new;
  ...
  //pairs_new = form_key_value_pair(fd,characteristic_table);
  //pairs = Write_to_pair(fd,characteristic_table);
 //pairs_new = Write_data(fd,characteristic_table);
 pairs_new = Write_data(fd,characteristic_table);
 /*while (pairs_new != NULL)
    {
      printf (" %s ====> ", pairs_new->key);
      if(strcmp(pairs_new->key,"Device Name")==0)
      {
          printf("%s",pairs_new->value);
      }
      else
      {
            if(pairs_new->value[1] == NULL)
            {
                strcpy(pairs_new->value, "NULL");
                printf("%s", pairs_new->value);
            }
            else
             {
                for (i=1; i<20;i++) 
			printf("%.2X ",pairs_new->value[i]);
                int j=0;
                for (i=0; i<20;i++)
                {
                    sprintf(&temp[j],"%.2X ",pairs_new->value[i]);
                    j=j+3;
                }
              // printf("temp : %s", temp);
               strncpy(pairs_new->value, temp,strlen(temp));
              // printf("\tpairs->value: %s",pairs->value);
            }
      }

     printf("\n");
      pairs_new = pairs_new->pNext;
    }*/
 ...
 while (pairs1 != NULL)
    {
      ...
      else
      {
			...
			printf("%.2X ",pairs1->value[i]);
                ...
                for (i=0; i<20;i++)
                {
                    sprintf(&temp[j],"%.2X ",pairs1->value[i]);
                    j=j+3;
                }
              // printf("temp : %s", temp);
  ...
  while (pairs != NULL)
    {
      ...
      else
      {
            if(pairs->value[1] == NULL)
            {
             ...
             {
                for (i=1; i<20;i++) 
			printf("%.2X ",pairs->value[i]);
                int j=0;
                for (i=0; i<20;i++)
                {
                    sprintf(&temp[j],"%.2X ",pairs->value[i]);
                    j=j+3;
                }
              // printf("temp : %s", temp);
