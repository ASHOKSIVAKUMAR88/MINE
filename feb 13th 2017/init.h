...
//	json_object* json_local_array = json_object_new_array();
	json_object_object_add(json_local,"resources",json_object_new_int(b));
	for(int i=0;i<res_new.numOfResources;i++)
	{
		json_object_object_add(json_local,res_new.ident[i].res_id,json_object_new_string(res_new.ident[i].res_name));		
		}
	return json_local;
}///dm_discovery
