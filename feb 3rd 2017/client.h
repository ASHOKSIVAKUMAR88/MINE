...
/////////////////////////init definition/////////////////////////////////////////////////////
void platformResource(OCRepresentation rep,OCPlatformInfo rep1)
{
	cout<<"in platform"<<endl;
	for(auto &pl: rep.getResourceTypes())//getResourceInterfaces()
		cout<<"\t\t\t\t"<<pl<<endl;
	for(auto &pl: rep.getResourceInterfaces())//getResourceInterfaces()
		cout<<"\t\t\t\t"<<pl<<endl;
	

		cout<<"\t\t\t"<<rep.numberOfAttributes()<<endl;
		cout<<"\t\t\t"<<rep.getHost()<<endl;
		cout<<"\t\t\t"<<rep.getUri()<<endl;
		cout<<"\t\t\t"<<rep1.platformID<<endl;
		//cout<<"\t\t\t"<<rep.begin()<<endl;
		//cout<<"\t\t\t"<<rep.end()<<endl;
...
int init(void)
{
    ...
    try
    {
        requestURI << OC_RSRVD_WELL_KNOWN_URI;////////"/oic/res"
	 // requestURI << OC_RSRVD_DEVICE_URI ;////"/oic/d"
		r << OC_RSRVD_PLATFORM_URI; //"/oic/p"
	//OCPlatform::getPlatformInfo("", r.str(), CT_IP_USE_V4, &platformResource, OC::QualityOfService::LowQos);
	///OCPlatform::findResource("192.168.4.146:0","",CT_IP_USE_V4 ,&foundResource);
	OCPlatform::findResource("",requestURI.str(),CT_IP_USE_V4 ,&foundResource);

	//OCPlatform::findResource("","",CT_ADAPTER_IP ,&foundResource);
...
//void onGet(const OCRepresentation& rep)
{
    try
    {
        if(1)//eCode == OC_STACK_OK)
        {
	map<string,AttributeValue> div;
	div = rep.getValues();
	int a[10],i;
	string g;

	//template<typename T> g;
	for(map<string,AttributeValue>::iterator it = div.begin(); it!=div.end();++it)
	{

	cout<<it->first<<"::"<<endl;
	//rep.getValue(it->first,g);
	cout<<rep.getValueToString(it->first)<<endl;
//	cout<<g<<endl;
...
void getLightRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
       ...
       QueryParamsMap test;
        resource->get(test, &onGet);///////////iotivity get request call/////////////////////////////
