...
class LightResource
{
	...
	public:
	    /// Constructor
	    LightResource(std::string s) :m_name("ASHOK SIVA KUMAR PUJALA"), m_state(false), m_power(34),m_string("enter data"),m_lightUri(s),m_resourceHandle(nullptr)
		{
		...
		m_lightRep.setValue("input_string",m_string);
		    }
		...
		/*OCPresenceHandle oc_presenceHandle;
		OCStackResult result = OCPlatform::subscribePresence(oc_presenceHandle, resourceURI, resourceTypeName,resourceInterface, cb, 																resourceProperty);*/
 	   ...
 	   OCStackResult createResource1()
    		{	
       		 // URI of the resource
		cout<<"second resource"<<endl;
	        std::string resourceURI ="/a/good";//m_lightUri;//"/a/light1";//m_lightUri;//"/a/ashok";
        	// resource type name. In this case, it is light
        	std::string resourceTypeName = "core.light";
        	// resource interface.
        	std::string resourceInterface = LINK_INTERFACE;//DEFAULT_INTERFACE;
        	...
        	// This will internally create and register the resource.
        	OCStackResult result = OCPlatform::registerResource(resHandle, resourceURI, resourceTypeName,resourceInterface, cb, resourceProperty);
	    ...
	    OCResourceHandle getHandle()    	////////////////function getHandle()
    	{	
   ...
   void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("input_string", m_string))
            {
                cout << "\t\t\t\t" << "input_string: " << m_string << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "state not found in the representation" << endl;
    ...
    OCRepresentation post(OCRepresentation& rep)
    {
        ...
        if(first)
        {
            ...
            if(OC_STACK_OK == createResource1())
            {
                ...
                rep1.setValue("createduri", m_lightUri);//std::string("a/light"));//m_lightUri));//"/a/ashok"));
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            if(requestType == "GET")
            {
                ...
                else // normal response case.
                {
                    ...
                    if(OC_STACK_OK == OCPlatform::sendResponse(pResponse))//sending resource values
                    {
                        ehResult = OC_EH_OK;
                    }
                }
...
{
    std::cout << std::endl;
    std::cout << "Usage : siresourceresourcempleserver <value>\n";
...
int main(int argc, char* argv[])
{
  ...
  //  OCPersistentStorage ps {client_open, fread, fwrite, fclose, unlink };
    ...
    PlatformConfig cfg {
        ...
        OC::QualityOfService::LowQos,NULL
        //&ps
    };
...
*/
    try
    {
        ...
        LightResource myLight("/a/ashok");
	LightResource mine("/a/votarytech");
	...
	myLight.createResource();
        ...
        myLight.addInterface(std::string(DEFAULT_INTERFACE));///LINK_INTERFACE;
	mine.addType(std::string("core.dimlight"));
        mine.addInterface(std::string(DEFAULT_INTERFACE));
